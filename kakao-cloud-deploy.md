...
반려동물 집밥원료 공공데이터 API 사용 시:

```env
USE_PET_FOOD_PUBLIC_DATA=true
PUBLIC_DATA_PET_FOOD_API_URL=발급받은_API_엔드포인트
PUBLIC_DATA_PET_FOOD_SERVICE_KEY=발급받은_농사로_API_키
```

로컬 시연용 샘플 fallback만 사용할 때:

```env
USE_PET_FOOD_PUBLIC_DATA=false
```

`PUBLIC_DATA_SERVICE_KEY`는 GitHub에 커밋하지 말고 카카오클라우드 환경 변수 또는 secret으로 등록합니다.
...
- 집밥원료 API가 꺼져 있거나 실패하면 `/app/data/petFoodIngredients.sample.json`을 사용합니다.
